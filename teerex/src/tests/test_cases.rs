...
fn set_security_flags_works() {
	new_test_ext().execute_with(|| {
		...
		assert!(<AllowSkippingAttestation<Test>>::get());
		assert!(!<SgxAllowDebugMode<Test>>::get());
...
fn add_and_remove_dcap_proxied_enclave_works() {
	new_test_ext().execute_with(|| {
		...
		let instance_address = EnclaveInstanceAddress {
			...
			signer: AnySigner::from(TEST1_DCAP_QUOTE_SIGNER),
		};
...
fn skip_attestation_add_sovereign_enclave_works_if_allowed() {
	new_test_ext().execute_with(|| {
		...
		let MultiEnclave::Sgx(sgx_enclave) = Teerex::sovereign_enclaves(&alice).unwrap();
		assert_eq!(sgx_enclave.attestation_method, SgxAttestationMethod::Skip { proxied: false });
...
fn skip_attestation_add_proxied_enclave_works_if_allowed() {
	new_test_ext().execute_with(|| {
		...
		let MultiEnclave::Sgx(sgx_enclave) = Teerex::proxied_enclaves(&instance_address).unwrap();
		assert_eq!(sgx_enclave.attestation_method, SgxAttestationMethod::Skip { proxied: true });
